...
describe 'print the Bank Statment' do
  ...
  xit 'print the bank statment with transaction history in reverse order' do
    date = Time.now.strftime('%d/%m/%Y')
    account.deposit(100.00)
    account.withdraw(20.00)
    expect(account.print_statment).to eq
    "date || credit || debit || balance\n
    #{date} || || 20.00 || 80.00\n
    #{date} || 100.00 || || 100.00"
  end

  xit 'print the bank statment with transaction on different days' do
